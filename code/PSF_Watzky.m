...
function signal = PSF_Watzky(n, m, c)
%% Inputs / parameters
...
%% Compute PSF
modes = [n(:), m(:), c(:)];
PSF = SLM_PSF(x_mm, y_mm, z_mm, alpha, k, f_mm, fwhm_pupil_mm, n_medium, nTheta, nPhi, N, modes);
signal = 10^(-20) * sum(PSF, "all") / (length(PSF)^2); % Summed intensity averaged out
% PSF = RW_on_grid(x_mm, y_mm, z_mm, alpha, k, f_mm, fwhm_pupil_mm, n_medium, nTheta, nPhi, aberrationFunc, N); % w/ aberration function

%% Plot PSF: axial and lateral

% Not worrying about z for signal intensity
